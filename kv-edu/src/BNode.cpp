...
{
    return ReadTypeAt<uint16_t>(dataPtr, NUM_CELLS_OFFSET);
}
...
uint32_t BNode::GetRightMostChild()
{
    ...
    return ReadTypeAt<uint32_t>(dataPtr, RIGHT_MOST_CHILD_OFFSET);
}

uint16_t BNode::GetCellOffset(size_t idx)
{
    return ReadTypeAt<uint16_t>(dataPtr, CELL_PTRS_OFFSET + idx * CELL_PTR_SIZE);
}

void BNode::SetType(BNodeType type)
{
    WriteTypeAt(dataPtr, BNODE_TYPE_OFFSET, type);
}

void BNode::SetNumCells(uint16_t numCells)
{
    WriteTypeAt(dataPtr, NUM_CELLS_OFFSET, numCells);
}

void BNode::SetFreeSpacePtr(uint16_t ptr)
{
    WriteTypeAt(dataPtr, FREE_SPACE_PTR_OFFSET, ptr);
}

void BNode::SetRightMostChild(uint32_t value)
{
    WriteTypeAt(dataPtr, RIGHT_MOST_CHILD_OFFSET, value);
}

void BNode::SetCellOffset(size_t idx, uint16_t value)
{
    uint32_t offset = CELL_PTRS_OFFSET + idx * CELL_PTR_SIZE;
    WriteTypeAt<uint16_t>(dataPtr, offset, value);
}